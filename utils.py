...
def prepare_data(sess, dataset):
  ...
  if FLAGS.is_train:
    ...
    data_dir = os.path.join(os.sep, (os.path.join(os.getcwd(), dataset)), "Set14")
    data = glob.glob(os.path.join(data_dir, "*.bmp"))
...
def merge(images, size):
  ...
  for idx, image in enumerate(images):
    ...
    img[int(j*h/3):int(j*h/3+h-x), int(i*w/3):int(i*w/3+w-x), :] = image[x:,x:,:]
    maxx,maxy=j*h/3+h-x,i*w/3+w-x

  return img[0:int(maxx-x),0:int(maxy-x)  ,:]
